...
function setupLogging() {
  try {
    ...
    console.log('Logging configurado. Log en:', logPath);
  } catch (err) {
    console.error('Error configurando logging:', err);
  }
}
...
if (!gotLock) {
  ...
  function startBackend() {
    return new Promise((resolve, reject) => {
      ...
      backendApp.post('/cars/:id/expenses', (req, res) => {
        ...
        stmt.run(req.params.id, amount, category, description, date, kilometers, photosStr, function(err) {
          if (err) return res.status(500).json({ error: err.message });
          res.json({ id: this.lastID });
        });
        stmt.finalize();
